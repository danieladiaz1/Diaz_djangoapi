...
from app_proyecto_parques.models import Zona
...
class CRUDZonas:
    """
    Clase CRUD para el modelo Zona (d.zonas).
    Geometría: POLYGON, SRID 25830.
    ...
    Restricciones espaciales aplicadas antes de insert/update:
      ...
      - ST_Intersects: rechaza si la geometría intersecta con zonas existentes.
    """

    def _apply_spatial_constraints(self, geom_wkt: str, exclude_id=None):
        """
        Aplica ST_SnapToGrid, ST_IsValid y ST_Intersects sobre el polígono entrante.
        ...
        with connection.cursor() as cursor:
            ...
            # 3. Rechazo si intersecta con polígonos existentes
            ...
            if intersects:
                return None, "La geometría intersecta con una zona ya existente (ST_Intersects = True)"
    ...
    def insert(self, data: dict) -> dict:
        ...
        data: {
            'nombre': str, 'descripcion': str, 'capacidad': int,
            'abierto': bool, 'area': float, 'geom': str (WKT POLYGON 25830)
        }
        """
        try:
            ...
            return {
                "ok": True,
                "message": f"Zona insertada correctamente con id={instance.pk}",
    ...
    def delete(self, data: dict) -> dict:
        ...
        try:
            ...
            instance = Zona.objects.get(pk=pk)
            d = _instance_to_dict(instance)
            instance.delete()
            return {"ok": True, "message": f"Zona con id={pk} eliminada correctamente", "data": [d]}
        except Zona.DoesNotExist:
            return {"ok": False, "message": f"Zona con id={data.get('id')} no encontrada", "data": None}
        except Exception as e:
    ...
    def update(self, data: dict) -> dict:
        ...
        try:
            ...
            return {
                "ok": True,
                "message": f"Zona con id={pk} actualizada correctamente",
                "data": [_instance_to_dict(instance)],
            }
        except Zona.DoesNotExist:
            return {"ok": False, "message": f"Zona con id={data.get('id')} no encontrada", "data": None}
    ...
    def selectAsDicts(self, data: dict = None) -> dict:
        ...
        try:
            ...
            return {"ok": True, "message": f"{len(result)} zona(s) encontrada(s)", "data": result}
        except Exception as e:
    ...
    def selectAsTuples(self, data: dict = None) -> dict:
        ...
        try:
            ...
            if data and data.get('id') is not None:
                qs = qs.filter(pk=data['id'])
            tuples = list(qs.values_list('id', 'nombre', 'descripcion', 'capacidad', 'abierto', 'area', 'geom'))
            return {"ok": True, "message": f"{len(tuples)} zona(s) encontrada(s)", "data": tuples}
        except Exception as e:
