...
from core.myLib.generalModule import stringListToString
...
def getUserGroups(user: User):
    """
    Gets a list of the names of the user groups (list of strings). The user is an object of the
        django.contrib.auth.models.User class
    """
    l = user.groups.values_list('name',flat = True) # QuerySet Object
    return list(l)

def getUserGroupsToString(user: User):
    """
    Gets a single string with the user groups that the user belongs, separated by commas.
    """
    return stringListToString(getUserGroups(user))
...
#35.21 Controlar acceso a las vistas mediante la url, o el nombre de la vista
class CheckAccessToView():
    ...
    Ejemplo de uso:   
    CheckAccessToView.__init__(self, request,self.__class__.__name__)
    ...
    no_perm_json_response=JsonResponse({'ok':False, 'message':'You can not access to this operation', 'data':None})
    def __init__(self, request, viewClassName):
        APP_LABEL='SOLUCIONAR LO DEL NOMBRE DE LA APP'
...
class CheckAccessToUrl():
    ...
    un JsonResponse de falta de permisos, aunque el usuario esté autenticado. El json es el de siempre:
        {'ok':False, 'message':'You can not access to this operation', 'data':None}
    ...
    CheckAccessToUrl.__init__(self, request)
    if self.no_perm_to_use_view:
        return self.no_perm_json_response
    """
    no_perm_to_use_view=True
...
def create_permissions(permissions_list):
    """
    Crea permisos en la tabla auth_permission.
    Los permisos quedan asociados a la app auth. Para saber si tiene el perimiso
    el código es auth.codename

    Args:
        permissions_list (list): Lista de diccionarios, cada uno con los campos 'name' y 'codename'.

    Ejemplo:
        permissions = [
            {'name': 'Can view report', 'codename': 'can_view_report'},
            {'name': 'Can edit settings', 'codename': 'can_edit_settings'},
        ]
        create_permissions(permissions)
    
    managePermissions.addPermissionsToGroupName('gestor_municipal',['can_view_report', 'can_edit_settings'])
    managePermissions.addUserToGroup('j.cami2010@hotmail.com','gestor_municipal')

    joa=User.objects.get(username='joamona@cgf.upv.es')
    cami=User.objects.get(username='j.cami2010@hotmail.com')

    print(joa.has_perm('auth.can_view_report')) -> False si no es admin
    print(cami.has_perm('auth.can_view_report')) -> True
    """
    # Obtener el ContentType genérico para los permisos globales
    ...
    for permission_data in permissions_list:
        ...
        if not name or not codename:
            print(f"Saltando permiso inválido: {permission_data}")
            continue
        ...
        if created:
            print(f"Permiso creado: {name} ({codename})")
        else:
            print(f"El permiso ya existe: {name} ({codename})")
...
def user_belong_to_group(user:User, groupName:str) -> bool:
    return user.groups.filter(name=groupName).exists()

def user_belong_to_any_group_in_list(user:User, listOfNamesOfGroupsToCheck: list)->bool:
    l=getUserGroups(user)
    for groupToCkeck in listOfNamesOfGroupsToCheck:
        if groupToCkeck in l:
            return True
    return False

def user_belong_to_group_fromUsername(username:str, groupName:str) -> bool:
    user=User.objects.get(username=username)
    return user.groups.filter(name=groupName).exists()

def getGroupMembers(group_name:str)->list['str']:
    """
    Returns a list with the usernames of the users that belong to the group.
    """
